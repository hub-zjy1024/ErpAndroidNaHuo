...
public class TakePicYanhuoActivity extends SavedLoginInfoActivity implements View.OnClickListener, NoLeakHandler.NoLeakCallback {
    ...
    private Context mContext = TakePicYanhuoActivity.this;
        ...
        if (mHolder != null) {
            mHolder.addCallback(new SurfaceHolder.Callback() {
                ...
                public void surfaceCreated(SurfaceHolder holder) {
                    ...
                    //设置旋转角度
                    mCamera.setDisplayOrientation(getPreviewDegree((TakePicYanhuoActivity) mContext));
                    ...
                    try {
                        ...
                        Log.e("zjy", "TakePicYanhuoActivity->surfaceCreated(): mCamera.preview==" + width1 + "\t" + height1);
                        Log.e("zjy", "TakePicYanhuoActivity->surfaceCreated(): Screen==" + sw + "\t" + sh + "xx" + density);
                        Point finalSize = getSuitablePreviewSize(parameters, sw, sh);
                        if (finalSize != null) {
                            Log.e("zjy", "TakePicYanhuoActivity->surfaceCreated():final.preview==" + finalSize.x + "\t" + finalSize.y);
                        ...
                        if (cameraSp.getInt("width", -1) != -1) {
                            ...
                            Log.e("zjy", "TakePicYanhuoActivity.java->surfaceCreated(): ==readCacheSize width" + width + "\t" + height);
                            parameters.setPictureSize(width, height);
                            try {
                                mCamera.setParameters(parameters);
                            } catch (RuntimeException e) {
                                MyApp.myLogger.writeError(e, "TakePicYanhuoActivity--setParameters");
                                e.printStackTrace();
                            }
                        } else {
                ...
                public void surfaceDestroyed(SurfaceHolder holder) {
                    Log.e("zjy", "TakePicYanhuoActivity->surfaceDestroyed(): surface destroyed==");
                    auto.stop();
                    releaseCamera();
                }
            });
        ...
        if (picSizes.size() > 0) {
            ...
            dialog.setNegativeButton("完成", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    Log.e("zjy", "TakePicYanhuoActivity.java->selectSize: width==" + width + "\t" + height);
                    parameters.setPictureSize(width, height);
                    mCamera.setParameters(parameters);
                }
            });
            ...
            dialog.setPositiveButton("设为默认尺寸", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    Log.e("zjy", "TakePicYanhuoActivity.java->onClick(): default size pos==" + itemPosition);
    ...
    private void attachToSensor(OrientationEventListener mOrientationListener) {
        if (mOrientationListener != null) {
            if (mOrientationListener.canDetectOrientation()) {
                ...
                MyApp.myLogger.writeError(TakePicYanhuoActivity.class, "获取相机方向失败,Detect fail");
    ...
    public boolean getInsertResultMain(String remoteName, String insertPath) throws IOException, XmlPullParserException {
        ...
        if ("caigou".equals(flag)) {
            ...
            result = setChukuPicInfo(WebserviceUtils.WebServiceCheckWord, cid, did, Integer
    ...
    public String getUploadRemotePath() {
        ...
        if (flag != null && flag.equals("caigou")) {
            remotePath = UploadUtils.getCaigouRemotePath(pid);
        } else {
            remotePath = UploadUtils.getChukuPath(pid);
        }
    ...
    public void getUrlAndFtp() {
        if (flag != null && flag.equals("caigou")) {
            ...
            ftpUtil = new FTPUtils(FtpManager.mainAddress, FtpManager.mainName,
                    FtpManager.mainPwd);
        } else {
            mUrl = kfFTP;
    ...
    public InputStream getTransformedImg(int cRotate, byte[] picData, Bitmap wtBmp) throws IOException {
        ...
        Bitmap waterBitmap = ImageWaterUtils.createWaterMaskRightBottom(mContext, photo,
                wtBmp);
        Bitmap TextBitmap = ImageWaterUtils.drawTextToRightTopByDef(mContext, waterBitmap, pid,
                (int) (photo.getWidth() * 0.015));
    ...
    public String setChukuPicInfo(String checkWord, int cid, int did, int uid, String pid, String fileName, String filePath,
                                  String stypeID) throws IOException, XmlPullParserException {
        return ChuKuServer.SetInsertPicInfo(checkWord, cid, did, uid, pid, fileName, filePath, stypeID);
    }
